...
public interface TacheRepository extends JpaRepository<Tache, Integer>, JpaSpecificationExecutor<Tache> {
    ...
    @Query(value = "select username from Utilisateur a inner join work_on b on a.ID_USER = b.ID_USER where b.id_task = ?1", nativeQuery = true)
    List<String> getTaskUsers(int id);
}